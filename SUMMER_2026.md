...
- **The system prompt is versioned, not hard-coded.** `produce_grading_prompts` (migration `00010`) is append-only; the newest row is active and is read per grading run. Super admins edit it on the admin System page (`/admin/system`, `GET`/`PUT /api/admin/system/grading-prompt`); saving appends a version. `models.DefaultGradingSystemPrompt` seeds the first row at startup (`EnsureProduceGradingPrompt`) and is the fallback if the table can't be read.
...
- Both queries still bucket time with `route LIKE` — T16 replaces that with the `phase` column in one place each.
