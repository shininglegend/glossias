...
- **Schema lives in one file:** `src/pkg/database/schema.sql` (no `migrations/` files despite CLAUDE.md saying so). SQLC queries in `src/pkg/database/queries/*.sql`, regenerated with `sqlc generate` into `src/pkg/generated/db/` (never hand-edit).
- **Storage:** Supabase Storage via `storage-go`; bucket `"audio-files"`, signed URLs generated on demand in `src/pkg/models/audio_files.go`. No image storage exists yet.
...
1. **Schema evolution without a migration system.** The schema is applied via `CREATE TABLE IF NOT EXISTS` from `src/pkg/database/schema.sql` on every startup; `migrations/*.sql` is gitignored. New tables happen to work with this approach, but any ALTER to an existing table (e.g., a `stories` column) will **silently not apply** to databases where the table already exists. Adopt a real migration tool (`golang-migrate` or `goose`) and version-control migrations *before* the first S26 schema change (developer_review.md #17).
