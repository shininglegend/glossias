...
- **Storage:** Supabase Storage via `storage-go`; bucket `"audio-files"`, signed URLs generated on demand in `src/pkg/models/audio_files.go`. Images mirror this in the `"images"` bucket via `src/pkg/models/story_images.go` (T4).
...
- Admin: extend the annotations/admin editor (`src/admin/stories/handler.go`, `frontend/app/routes/admin.stories.$id.*`) with target-vocab selection + audio/image upload per word.
...
**Open question for T7 — two ways to attach an image.** T3 and T4 landed independently and each followed the spec, but they overlap: T4 added a `story_images(story_id, file_path, file_bucket, label)` table mirroring `line_audio_files`, while T3 added path/bucket columns directly on `target_vocabulary` and `recall_sentences` as F2/F3 specified. Both can name the same file. T7 should pick one before authoring begins — recommend the T3 columns as the source of truth for *which* image belongs to a target word or recall sentence (the relationship is 1:1 and a FK-bearing column expresses that better than a label string), using T4's upload/confirm/signed-read flow to move the bytes. If `story_images` is kept as the registry instead, the T3 columns should be dropped in a follow-up migration rather than left to drift.
