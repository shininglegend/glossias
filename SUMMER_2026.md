...
- [X] **T12 — Produce phase (frontend + submit endpoint).** Timed two-segment translation UI, reference reveal, explanation popup, and `POST /produce` storing submissions (ungraded). *Depends on: T2, T3, T5.* *Complexity: 6/10 · ~10 files.*
- [X] **T13 — AI grading.** Anthropic SDK integration, grading prompt + iteration on sample answers, fail-open behavior, and rate limiting on the submit path. *Depends on: T12 (and developer_review.md #2 rate-limiter fix).* *Complexity: 7/10 · ~8 files — small diff, high risk: external API, cost, latency, prompt iteration.*
- [X] **T14 — Score page rework.** New accuracy categories, five-phase time breakdown, and incomplete-detection for the new phases in `stories-score.go` + `StoriesScore.tsx`. Also bring the admin per-student report (`GetStoryStudentPerformance` in `scores.sql`, rendered by `CourseStudentPerformance.tsx`) onto the same categories: it still reports only vocab/grammar/translation and merely time-buckets identify/produce/recall — see "Admin student report" under Score page changes. *Depends on: T10, T11, T12 (T13 for graded produce scores).* *Complexity: 6/10 · ~11 files — must tolerate mixed-generation data.*
- [X] **T15 — Per-student progress reset.** `DELETE /api/admin/stories/{id}/students/{userId}/progress?phase=` wipes one student's answer/submission/time rows for a story (or one phase) so they can redo it; "Reset…" button per row in the admin performance table. Also moved the report from the misfiled `/api/admin/courses/{storyId}/student-performance` to `GET /api/admin/stories/{id}/students`. *Depends on: T3.* *Complexity: 4/10 · ~10 files.*
- [X] **T16 — Time-tracking phase column + per-student drill-down.** (a) Add a `phase` column to `user_time_tracking`, set from the route at write time in `src/apis/timetracking.go`, and switch both `GetStoryStudentPerformance` and the T15 reset (`phaseSpecs.routePatterns` in `models/reset_progress.go`) from `route LIKE '%vocab%'` substring matching to it. (b) Add `GET /api/admin/stories/{id}/students/{userId}` returning that student's actual answers/submissions per phase, and a drill-down page linked from each row of the performance table. *Depends on: T14, T15.* *Complexity: 5/10 · ~10 files.*
...
**As built (T10)** — `src/apis/handlers/stories-identify.go`, `frontend/app/components/StoriesIdentify.tsx`, `story-components/IdentifyQuizModal.tsx`, `lib/identifyMachine.ts`. Decisions downstream tasks depend on:

- **`GET /api/stories/:id/identify`** returns `lines[]` (`text` segments with a new `"target"` type carrying `target_vocab_id`, plus `target_vocab_ids` per line), `target_words[]` (id, lexical form, signed `audio_url` / `image_url` via `SignTargetVocabularyURLs`), `audio_urls` (signed `complete`-label narration keyed by 1-based line number, so the payload feeds `useAudioPlayer` directly), plus the user's progress: `correct_picks[]` (`{line_index, target_vocab_id}` quizzes already answered correctly) and `completed`. A story with no target vocabulary returns an empty `target_words` list and the phase degrades to plain narration with a notice — it does not 500. Narration signing failures are logged and yield an empty `audio_urls`, matching how the Vocab and Translate pages tolerate them.
- **`POST /api/stories/:id/check-identify`** takes `{line_index (0-based), target_vocab_id, selected_target_vocab_id}` and answers `{correct}`. Both IDs must belong to the story and the target must actually occur on that line (checked against `GetTargetVocabularyOccurrences`), otherwise 400 — a client cannot log a correct pick for a line the word is not on. Line numbers are stored 1-based via `models.SaveIdentifyAnswer`, consistent with vocab answers. Every pick is logged, so `GetUserStoryIdentifySummary` counts retries as incorrect attempts; T14 should score with `CalculateScoreWithRetriesAllowed`.
- **Segment rendering:** `VocabTextRenderer` gained a `"target"` branch (amber, underlined, `data-target-vocab-id`), so `StoryLine` is reused unchanged. Occurrence offsets are rune-based; overlapping or out-of-range occurrences are dropped rather than corrupting the text (`buildIdentifyLines`, unit-tested).
- **Flow is a pure reducer** (`identifyMachine.ts`: `idle → playing ⇄ paused`, `playing → quiz → replaying → playing`, `→ complete`), following T9's command pattern. The hook pauses via `pauseOnLines = {lines with targets}` and reports the pause through a new **`onPauseAfterLine`** option on `useAudioPlayer` (T6 style: optional, default unchanged, tested) — inferring "the line ended" from `isPlaying` flips would re-open the quiz after the replay. A line with several target words quizzes them in turn before the single replay. Pause/resume is only possible while playing (not during a quiz or replay); the quiz dialog cannot be dismissed by Escape or backdrop — only the right picture closes it. Wrong picks are disabled in place; pictures are shuffled once per quiz so they do not move after a wrong pick.
- **No redo; reload resumes.** Completion is *derived*, not stored: the phase is complete when every target-word occurrence in the story has a correct pick (`identifyProgress` in `stories-identify.go`, also used by navigation's `isIdentifyCompleted`). No migration was needed — every correct pick is already persisted as it happens. A finished phase opens directly in the complete state with no Start button; an unfinished one resumes at the last line with a correct pick and skips quizzes already answered (`pauseOnLines` is recomputed from the picks, and `LINE_ENDED` filters answered targets). Because completion is derived, a word an author adds later re-opens the phase for exactly that word's occurrences, and a story with no target words is never "complete" this way — it just plays through. The machine ignores `LINE_CHANGED` unless playing, since the hook reports line 0 on mount and would otherwise clobber the resume point.
- **Picture options** are the story's own five target words, so a story with fewer than five words simply shows fewer options — the readiness report (T7) is what flags that, not this phase.
...
- **Progress is saved per reveal and restored on load.** `PUT /api/stories/:id/translate?lines=[…]` (0-indexed, merged server-side) is called after every reveal, and once more with `&complete=true` when the phase ends. `GET …/translate` now returns `requested_lines` and `completed` (replacing the old `has_translation` boolean); the machine is seeded from them, counts those lines toward the quota and cap, and a reload resumes from the last translated line rather than the top. A completed phase opens in its finished state (requested translations shown, Continue button) and cannot be replayed. Because a row now exists mid-phase, migration `00005` adds `translation_requests.completed_at` (backfilled from `created_at`), and every "translate phase done" check — navigation, the score page, and the course-performance SQL — reads `completed_at IS NOT NULL` via `models.TranslationRequestCompleted` instead of "row exists".
...
**Admin student report (part of T14).** `GetStoryStudentPerformance` (`src/pkg/database/queries/scores.sql`) → `models.GetStoryStudentPerformance` (`course_performance.go`) → `CourseStudentPerformance.tsx` is the instructor-facing equivalent of the score page and has the same staleness: it reports vocab/grammar accuracy and translation completion, and only *time* for identify/produce/recall. Bring it onto the same five-phase categories as the score page (identify accuracy, produce AI score/graded status, recall attempts), and while touching the query replace its eight correlated scalar subqueries per student (`LEFT JOIN LATERAL (SELECT (SELECT COUNT(*) …) …)`) with per-table `GROUP BY user_id` aggregates joined once — fine at 30 students, but it is O(students × tables) round trips inside one statement. The CSV export and the row sort order (currently vocab+grammar correct) need the same update. Per-student progress reset (T15) already lives on the same table; the drill-down and the `phase` column that removes `route LIKE` bucketing are T16.

**As built (T14).** One scoring rule feeds both pages: `models.ComputePhaseScores(summary, totals)` in `src/pkg/models/phase_scores.go` turns raw counts into Identify/Recall accuracy (`CalculateScoreWithRetriesAllowed`), Recall attempts (`(correct+incorrect)/sentences`), the Produce AI average, and an **overall** that averages only the phases the story has — Identify and Recall when authored, Produce once at least one segment is graded (ungraded is "pending", not 0). A story with none of those (pre-five-phase content) falls back to the legacy vocab+grammar score, which is the mixed-generation tolerance. Decisions downstream tasks depend on:

- **Score page** (`GetScoresData`): 3 queries beyond the story load — `GetUserStoryPageCompletion` (reused for the per-phase totals and completion flags, so navigation and the score page can never disagree), the new `GetUserStoryScoreSummary` (every answer count in one statement), and time tracking. A phase blocks the page only when the story has content for it; Identify/Produce/Recall report `no_data` (never started) or `incomplete` (started, not finished) so the button reads Start/Continue. Legacy vocab/grammar never block and their cards render only when attempts exist. `ScoreData` gained `*_total` fields so the frontend knows which cards to draw.
- **Admin report** (`GetStoryStudentPerformance`): rewritten as `GROUP BY user_id` CTEs joined once (one per answer table, Produce as latest-per-segment, time in one pass), plus `GetStoryPhaseTotals` for the denominators — 2 queries per request regardless of class size. Rows come back sorted best overall first (ties: least time, then email) so the table and CSV share the order; the frontend no longer sorts. Legacy Vocab/Grammar columns appear only when some row has legacy attempts. Response fields are on `models.CourseStudentPerformance`; `convertToInt32` and the `requested_lines` type switch are gone because the query now casts every column.
- ~~Both queries still bucket time with `route LIKE` — T16 replaces that with the `phase` column in one place each.~~ Done by T16.

**As built (T16).** (a) Migration `00012` adds `user_time_tracking.phase`, backfilled with the same CASE the queries used; `models.PhaseFromRoute` (`time-tracking.go`) sets it at write time in `RecordTimeTracking`, and the backfill CASE must stay in sync with it. `GetUserStoryTimeTracking`, the admin report's `time_stats` CTE, and the T15 reset (`phaseSpec.timePhase` + `DeleteUserStoryTimeTrackingByPhase`) all match on `phase = '…'` now; rows whose route matches no phase stay NULL and count only toward per-story totals, as before. Legacy `audio` routes backfill into the `video` bucket, so `ResetVideo` needs only one delete. (b) `GET /api/admin/stories/{id}/students/{userId}` (`students.go` → `models.GetStudentStoryDrilldown`, 7 queries flat) returns every Identify pick (target + picked word), translate status with requested lines, every Produce submission per segment (not just the latest) with AI score/feedback, and Recall attempts rebuilt from the flat answer log (`groupRecallAttempts` splits on a repeated position, since per-row timestamps within an attempt differ). Frontend: `admin/stories/:id/students/:userId` renders `StudentStoryDrilldown.tsx`; each student name in `CourseStudentPerformance.tsx` links to it.
